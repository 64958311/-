...
def encode_enc(clazzid: str, duration: int, objectId: str, otherinfo: str, jobid: str, userid: str,
               currentTimeSec: str):
    import hashlib
    data = "[{0}][{1}][{2}][{3}][{4}][{5}][{6}][0_{7}]".format(clazzid, userid, jobid, objectId,
                                                               int(currentTimeSec) * 1000, "d_yHJ!$pdA~5",
...
def sign_in(uname: str, password: str):
    ...
    sign_in_data = "fid=314&uname={0}&password={1}&refer=http%253A%252F%252Fi.mooc.chaoxing.com&t=true".format(uname,
                                                                                                               des_pwd(
                                                                                                                   password,
                                                                                                                   "u2oh6Vu^").decode(
                                                                                                                   'utf-8'))
    ...
    for course_unit in course_unit_list:
        ...
        for h3_item in h3_list:
            ...
            if chapter_status == "orange":
                print("--" * level, __list_get(h3_item.xpath("./a/span[@class='articlename']/@title")), "      ",
                      __list_get(h3_item.xpath("./a/span[@class='icon']/em/text()")))
...
def recursive_course_dict(course_unit_list, chapter_dict):
    for course_unit in course_unit_list:
        h3_list = course_unit.xpath("./h3")
        for h3_item in h3_list:
            chapter_dict.update({__list_get(h3_item.xpath("./a/span[@class='chapterNumber']/text()")) + __list_get(
                h3_item.xpath("./a/span[@class='articlename']/span[@class='chapterNumber']/text()")) + __list_get(
                h3_item.xpath("./a/span[@class='articlename']/@title")): __list_get(h3_item.xpath("./a/@href"))})
        chapter_item_list = course_unit.xpath("./div")
        if chapter_item_list:
...
def misson_audio(objectId, otherInfo, jobid, name, reportUrl, clazzId):
    ...
    elses = "/{0}?clazzId={1}&playingTime={2}&duration={2}&clipTime=0_{2}&objectId={3}&otherInfo={4}&jobid={5}&userid={6}&isdrag=0&view=pc&enc={7}&rt=1&dtype=Audio&_t={8}".format(
        dtoken, clazzId, duration, objectId, otherInfo, jobid, uid,
        encode_enc(clazzId, duration, objectId, otherInfo, jobid, uid, duration), int(time.time() * 1000))
...
def misson_video(objectId, otherInfo, jobid, name, reportUrl, clazzId):
    ...
    elses = "/{0}?clazzId={1}&playingTime={2}&duration={2}&clipTime=0_{2}&objectId={3}&otherInfo={4}&jobid={5}&userid={6}&isdrag=0&view=pc&enc={7}&rt=1&dtype=Video&_t={8}".format(
        dtoken, clazzId, duration, objectId, otherInfo, jobid, uid,
        encode_enc(clazzId, duration, objectId, otherInfo, jobid, uid, duration), int(time.time() * 1000))
    reportUrl_item = reportUrl + str(elses)
    video_url_list.append(reportUrl_item)
...
def misson_live(streamName, jobid, vdoid, courseId, chapterId, clazzid):
    src = 'https://live.chaoxing.com/courseLive/newpclive?streamName=' + streamName + '&vdoid=' + vdoid + '&width=630&height=530' + '&jobid=' + jobid + '&userId={0}&knowledgeid={1}&ut=s&clazzid={2}&courseid={3}'.format(
        uid, chapterId, clazzid, courseId)
    ...
    finish_url = "https://zhibo.chaoxing.com/live/saveCourseJob?courseId={0}&knowledgeId={1}&classId={2}&userId={3}&jobId={4}&token={5}".format(
        courseId, chapterId, clazzid, uid, jobid, token[0])
...
def misson_doucument(jobid, chapterId, courseid, clazzid, jtoken):
    ...
    url = 'https://mooc1-2.chaoxing.com/ananas/job/document?jobid={0}&knowledgeid={1}&courseid={2}&clazzid={3}&jtoken={4}&_dc=1607066762782'.format(
        jobid, chapterId, courseid, clazzid, jtoken)
...
def misson_book(jobid, chapterId, courseid, clazzid, jtoken):
    ...
    url = 'https://mooc1-2.chaoxing.com/ananas/job?jobid={0}&knowledgeid={1}&courseid={2}&clazzid={3}&jtoken={4}&_dc={5}'.format(
        jobid, chapterId, courseid, clazzid, jtoken, int(time.time() * 1000))
    multimedia_rsp = requests.get(url=url, headers=multimedia_headers)
    print(multimedia_rsp.text)
...
def misson_read(jobid, chapterId, courseid, clazzid, jtoken):
    ...
    url = 'https://mooc1-2.chaoxing.com/ananas/job/readv2?jobid={0}&knowledgeid={1}&courseid={2}&clazzid={3}&jtoken={4}&_dc={5}'.format(
        jobid, chapterId, courseid, clazzid, jtoken, int(time.time() * 1000))
    multimedia_rsp = requests.get(url=url, headers=multimedia_headers)
    print(multimedia_rsp.text)
...
def deal_misson(missons: list, class_cpi: str, mode: int):
    for chapter_mission_item in missons:
        ...
        for num in range(cardcount):
            try:
                print("num:", num)
                medias_url = "https://mooc1-2.chaoxing.com/knowledge/cards?clazzid={0}&courseid={1}&knowledgeid={2}&num={4}&ut=s&cpi={3}&v=20160407-1".format(
                    clazzId, courseId, chapterId, class_cpi, num)
    ...
    for media_item in result_json:
        ...
        if media_type == "video":
            ...
            if (media_item.get('property').get('module') == "insertaudio"):
                misson_audio(objectId=objectId, otherInfo=otherInfo, jobid=jobid, name=name,
                             reportUrl=data["defaults"]["reportUrl"], clazzId=clazzId)
            else:
                url_video = misson_video(objectId=objectId, otherInfo=otherInfo, jobid=jobid, name=name,
                                         reportUrl=data["defaults"]["reportUrl"], clazzId=clazzId)
            # multimedia_headers = {
            #     'Accept': '*/*',
            #     'Accept-Encoding': 'gzip, deflate, br',
...
def medias_download(medias):
    ...
    for media_item in medias:
        ...
        status_rsp = requests.get(
            url="https://mooc1-1.chaoxing.com/ananas/status/{}?k=&flag=normal&_dc=1600850935908".format(objectid),
...
def get_task_status(url: str):
    ...
    clazzId = chapter_data.get("clazzid")[0]
    sta_url = "https://stat2-ans.chaoxing.com/task/s/index?courseid={0}&cpi={1}&clazzid={2}&ut=s&".format(courseId, cpi,
                                                                                                          clazzId)
    rsp = requests.get(url=sta_url, headers=global_headers)
    rsp_html = etree.HTML(rsp.text)
    already_time = float(
        __list_get(re.findall("[0-9]+[.]?[0-9]?", __list_get(rsp_html.xpath("//div[@class='fl min']/span/text()")))))
    all_time = float(
        __list_get(re.findall("[0-9]+[.]?[0-9]?", __list_get(rsp_html.xpath("//p[@class='bottomC fs12']/text()")))))
    print(already_time, "/", all_time)
    chapterId = False
    if already_time < all_time:
        datal_url = "https://stat2-ans.chaoxing.com/task/s/progress/detail?clazzid={0}&courseid={1}&cpi={2}&ut=s&page=1&pageSize=16&status=0".format(
        ...
        for i in range(cardcount):
            try:
                medias_url = "https://mooc1-2.chaoxing.com/knowledge/cards?clazzid={0}&courseid={1}&knowledgeid={2}&num={4}&ut=s&cpi={3}&v=20160407-1".format(
                    clazzId, courseId, chapterId, cpi, i)
                medias_rsp = requests.get(url=medias_url, headers=global_headers)
                medias_HTML = etree.HTML(medias_rsp.text)
                medias_text = medias_HTML.xpath("//script[1]/text()")[0]
                pattern = re.compile(r"mArg = ({[\s\S]*)}catch")
                ...
                for media_item in datas["attachments"]:
                    ...
                    if media_type == "video":
                        ...
                        otherInfo = media_item.get("otherInfo")
                        name = media_item.get('property').get('name')
                        return VideoThread(misson_video(objectId=objectId, otherInfo=otherInfo, jobid=jobid, name=name,
                                                        reportUrl=datas["defaults"]["reportUrl"], clazzId=clazzId),
                                           name=name)
                    else:
                        continue
            except Exception as e:
...
class video_nomal_thread(threading.Thread):
    ...
    def url_replace(self, now_time: int) -> str:
        enc_tmp = encode_enc(self.clazzId, int(self.duration), self.objectId, self.otherInfo, self.jobid, self.uid,
                             str(now_time))
...
class Things():
    ...
    def misson_1(self):
        ...
        if enter == "":
            ...
            if len(video_url_list) == 0:
                ...
                speed = input(
                    "请选择节点的完成方式 1.立即完成(1秒即可完成视频任务点) 2.常规速度完成(完成时间与视频时间等长) :")
                while speed != "1" and speed != "2":
                    print("请输入正常的序号")
                    speed = input(
                        "请选择节点的完成方式 1.立即完成(1秒即可完成视音频任务点) 2.常规速度完成(完成时间与视音频时间等长) :")
                if speed == "1":
                ...
                if enter == "q":
                    ...
                    if len(video_url_list) == 0:
                        ...
                        while speed != "1" and speed != "2":
                            print("请输入正常的序号")
                            speed = input(
                                "请选择节点的完成方式 1.立即完成(1秒即可完成任务点) 2.常规速度完成(完成时间与视音频时间等长) :")
                        if speed == "1":
                            for item in video_url_list:
                                multimedia_headers = {
                                    'Accept': '*/*',
    ...
    def misson_5(self):
        ...
        threadPool = []
        print("正在读取 %s :" % ('建设工程法规'))
        isThread = get_task_status(
            'https://mooc1-2.chaoxing.com/visit/stucoursemiddle?courseid=207294662&clazzid=67164324&vc=1&cpi=186028059')
        if isThread:
...
def before_start() -> None:
    print("欢迎您使用超星工具, 本工具是针对超星(学习通)所编写的脚本工具")
    print("""
    ********************************************************************
    --------------------------------------------------------------------
    --------------------------------------------------------------------
